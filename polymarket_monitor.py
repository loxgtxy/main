...
ORDERBOOK_ENDPOINT_TEMPLATE = f"{API_BASE}/markets/{{market_id}}/orderbook"
DEFAULT_LIMIT = 30
...
def fetch_markets(active_only: bool = True, limit: int = 500) -> List[Dict[str, Any]]:
    ...
    data = _http_get(MARKETS_ENDPOINT, **params)
    if isinstance(data, dict) and "data" in data:
        return data["data"]
    if isinstance(data, list):  # fallback: older API shape
        return data
...
def find_market(
    ...
    """Search a market by slug or question keyword."""
    ...
    for market in markets:
        ...
        if slug and market_slug == slug:
            return market
        if question and question.lower() in question_text.lower():
...
def monitor_market(
    ...
    market = find_market(markets, slug=slug, question=keyword)
